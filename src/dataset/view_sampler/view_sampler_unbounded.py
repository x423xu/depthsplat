...
class ViewSamplerUnbounded(ViewSampler[ViewSamplerUnboundedCfg]):
    ...
    '''
    For unbounded sampling, the target views should be outside of context views.
    We primarily work on two+one context views, four target views
    '''
    def sample(
        self,
        scene: str,
        ...
        **kwargs,
        ...
        if self.stage == "test":
            ...
            # for unbounded setting, the target view should be outside of context views
            ...
            # )
            index_target = torch.randint(
                max(index_context_left-self.cfg.max_distance_to_context_views,0),
                min(index_context_right + self.cfg.max_distance_to_context_views,num_views),
                size=(self.cfg.num_target_views,),
                device=device,
            )
